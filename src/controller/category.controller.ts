...
export async function CreatCategoriesFile(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        ...
        const done = await Category.bulkCreate(results);
        res.status(201).json({ status: "success", message: "Categories created successfully", data: done });
