...
export const signupForgetPass = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (email) {
            ...
            if (EmailFind) {
                const createByPass = await genHash(NewPassword)
                const HandleUpdate = await User.update(
                    { password: createByPass },
                    { where: { email } }
                );
