...
export const cartUpdate = async (req: RequestWithUser, res: Response, next: NextFunction) => {
    try {
        ...
        console.log(carT_id, user_id, { updateFlag })
        if (!user_id) {
            return res.status(400).json({ status: "failed", message: "user id not valid" })
        }

        const cartUpdate: any = await Cart.findOne({ where: { user_id, id: carT_id }, raw: true });
        if (!cartUpdate) {
            res.status(500).json({
                status: "Please ! add the Items!",
                message: `Cart is empty this user`,
        ...
        if (cartUpdate.product_qty < 1) {
            await Cart.destroy({ where: { user_id, id: carT_id } });
...
// export const cartAllItems = async(res:Response,req:Request,next:NextFunction)=>{
//    const CartAll = 
// }