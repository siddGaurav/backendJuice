...
export async function addProduct(req: Request, res: Response, next: NextFunction) {
    try {
        const { product_name, product_price, product_desc, product_sku, product_benifit, product_size, product_unit, cat_id } = req.body;
        console.log(req.body)

        // const product_size = req.body.product_size ? JSON.parse(req.body.product_size) : [];
        ...
        console.log(err);
        res.status(400).json({ status: 'failed', message: 'Product not createdd', err: err });
    }
}
...
export async function productFileUplaoder(
    req: Request,
    res: Response,
    next: NextFunction
) {
    try {
            ...
            stream
                ...
                .on("data", (data) => {
                    ...
                    products.push({
                        ...
                        product_benifit: data.product_benifit,
        ...
        res.status(201).json({
            ...
            totalInserted: createdProducts.length,
        });
    } catch (err) {
        next(err);
    }
